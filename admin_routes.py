...
def register_admin(app, deps: dict):
    ...
    # ========== Operations panel (rich view + actions UI) ==========
    @admin_bp.route("/ops")
    @admin_required
    def ops():
        shift = int(get_shift_hours())
        ...
        return render_template(
            "ops.html",  # your ops template (admin ops panel)
    ...
    def set_shift():
        ...
        # prefer returning to ops if user came from there
    ...
    def servers_page():
        if request.method == "POST":
            ...
            if action == "create":
                ...
                _refresh_server_cache(SERVER_CONFIGS, M)
                try:
    ...
    def _refresh_server_cache(cache: dict[int, dict], Models):
        cache.clear()
        for r in Models.ServerConfig.query.all():
            try:
                gid = int(r.guild_id)
            except Exception:
                continue
